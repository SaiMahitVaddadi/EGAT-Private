...
class Properties:
    def __init__(self):
        self.elements_and_numbers()
        self.valence()
        self.bonds()
        self.charge()
        self.electron_deficiency()
        self.octet()
        self.charge()
        self.mass()
        self.radii()
    ...
    def elements_and_numbers(self):
        ...
        self.el_to_an = self.titleizedict(self.el_to_an)
        # atomic number to element (lower-case)
        self.an_to_el = {self.el_to_an[i]:i.lower() for i in self.el_to_an.keys() }
    ...
    def valence(self):
        ...
        self.el_valence = self.titleizedict(self.el_valence)

        # Used for determining number of valence electrons provided by each atom to a neutral molecule when calculating Lewis structures
        self.el_alt_valence = {  'h':1, 'he':2,\
                        'li':1, 'be':2,                                                                                                                'b':3,  'c':4,  'n':5,  'o':6,  'f':7, 'ne':8,\
                        'na':1, 'mg':2,                                                                                                               'al':3, 'si':4,  'p':10,  's':12, 'cl':7, 'ar':8,\
                        'k' :1, 'ca':2, 'sc':3, 'ti':4,  'v':5, 'cr':6, 'mn':7, 'fe':8, 'co':9, 'ni':10, 'cu':11, 'zn':12, 'ga':3, 'ge':4, 'as':5, 'se':6, 'br':7, 'kr':8,\
                        'rb':1, 'sr':2,  'y':None, 'zr':None, 'nb':None, 'mo':None, 'tc':None, 'ru':None, 'rh':None, 'pd':None, 'ag':None, 'cd':None, 'in':3, 'sn':4, 'sb':5, 'te':6,  'i':7, 'xe':8,\
                        'cs':1, 'ba':2, 'la':None, 'hf':None, 'ta':None,  'w':None, 're':None, 'os':None, 'ir':None, 'pt':None, 'au':None, 'hg':None, 'tl':3, 'pb':4, 'bi':5, 'po':6, 'at':7, 'rn':8  }        
        ...
        self.el_max_valence = self.lowercasedict(self.el_max_valence)

    def electron_deficiency(self):
        ...
        self.el_n_deficient = self.titleizedict(self.el_n_deficient)
    ...
    def octet(self):
        ...
        self.el_n_expand_octet = {  'h':2, 'he':2,\
                            'li':2, 'be':0,                                                                                                                'b':8,  'c':8,  'n':8,  'o':8,  'f':8, 'ne':8,\
                            'na':0, 'mg':0,                                                                                                               'al':8, 'si':8,  'p':8,  's':8, 'cl':8, 'ar':8,\
                            ...
                            'rb':0, 'sr':0,  'y':None, 'zr':None, 'nb':None, 'mo':None, 'tc':None, 'ru':None, 'rh':None, 'pd':None, 'ag':None, 'cd':None, 'in':8, 'sn':8, 'sb':8, 'te':8,  'i':8, 'xe':8,\
                            'cs':0, 'ba':0, 'la':None, 'hf':None, 'ta':None,  'w':None, 're':None, 'os':None, 'ir':None, 'pt':None, 'au':None, 'hg':None, 'tl':8, 'pb':8, 'bi':8, 'po':8, 'at':8, 'rn':8  }        
        self.el_n_expand_octet = self.titleizedict(self.el_n_expand_octet)
            
        # Used to determine is expanded octets are allowed when calculating Lewis structures
        self.el_expand_octet = { 'h':False, 'he':False,\
        ...
        self.el_expand_octet = self.titleizedict(self.el_expand_octet)

    def charge(self):
        # Electronegativity (Allen scale)
        self.el_en = { "h" :2.3,  "he":4.16,\
        ...
        self.el_en = self.titleizedict(self.el_en)

        # Polarizability ordering (for determining lewis structure)
        self.el_pol ={ "h" :4.5,  "he":1.38,\
        ...
        self.el_pol = self.titleizedict(self.el_pol)


    def mass(self):
        # Average atomic masses
        self.el_mass = {'H':1.00794,'He':4.002602,'Li':6.941,'Be':9.012182,'B':10.811,'C':12.011,'N':14.00674,'O':15.9994,'F':18.9984032,'Ne':20.1797,\
        ...
        self.lowercasedict(self.el_mass)
    ...
    def radii(self):
        ...
        self.el_radii = {  'H':0.39, 'He':0.849,\
                    ...
                    'default' : 0.7 }
        self.lowercasedict(self.el_radii)

    def bonds(self):
        # This dictionary is used to flagging problematic adjacency matrices by the table_generator function.
        self.el_max_bonds = {  'H':2,    'He':1,\
        ...
        self.lowercasedict(self.el_max_bonds)

        
        self.old_el_max_bonds = {  'H':0.354, 'He':0.849,\
              'Li':1.336, 'Be':1.074,                                                                                                                          'B':0.838,  'C':0.757,  'N':0.700,  'O':0.658,  'F':0.668, 'Ne':0.920,\
        ...
        self.lowercasedict(self.old_el_max_bonds)
