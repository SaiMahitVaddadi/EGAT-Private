...
class ChempropMoleculeFeaturizer:
    ...
    def GetNeighbors(self,ind):
        ...
        if self.params.neighbor == 'onlyH':
            ...
            NE_count = [neighbors.count('C'), neighbors.count('N'), neighbors.count('O')]
            ...
            NE_count = [neighbors.count(element) for element in self.properties.element_encode.keys() if element != 'H']
        else:
            NE_count = []
        return NE_count
...
class ChempropReactionFeaturizer:
    def _init__(self):
        pass