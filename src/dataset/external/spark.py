...
class PandasSaver:
    ...
    def SaveRowToSpark(self, index):
        try:
            self.Convert(index)
            row = Row(**self.info)
            self.df = self.df.union(self.spark.createDataFrame([row], self.schema))
        except Exception as e:
            print(self.root + '--'+ str(index) + ' failed')
            print(traceback.print_exc())

    def SaveInfoToSpark(self):
        self.spark = SparkSession.builder.appName("SaveInfoToSpark").getOrCreate()
        self.schema = StructType([StructField(key, StringType(), True) for key in self.info.keys()])
        self.df = self.spark.createDataFrame([], self.schema)
        for index in tqdm(self.data.index.tolist(), total=len(self.data.index.tolist()), desc="Saving to Spark DataFrame"):
            self.SaveRowToSpark(index)
        self.df.write.csv(os.path.join(self.params.data_path), header=True, mode='overwrite')
...
class PandasDataset(Dataset):
    ...
    def LoadSparkDataFrame(self):
        try:
            self.spark = SparkSession.builder.appName("LoadSparkDataFrame").getOrCreate()
            self.data = self.spark.read.csv(self.params.data_path, header=True, inferSchema=True)
        except Exception as e:
            print(f"Failed to load Spark DataFrame from {self.params.data_path}")
            print(traceback.print_exc())
