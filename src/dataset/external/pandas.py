...
class PandasSaver:
    ...
    def SaveInfoToPandas(self):
        ...
        for index in tqdm(self.data.index.tolist(), total=len(self.data.index.tolist()), desc="Saving to Pandas DataFrame"):
            self.SaveRowToPandas(index)
        self.df.to_csv(os.path.join(self.params.data_path), index=False)
...
class PandasDataset(Dataset):
    ...
    def LoadPandasDataFrame(self):
        try:
            self.data = pd.read_csv(self.params.data_path, 'data')
        except Exception as e:
            print(f"Failed to load Pandas DataFrame from {self.params.data_path}")
            print(traceback.print_exc())
