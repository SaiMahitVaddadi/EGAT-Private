...
class PolarsSaver:
    ...
    def SaveRowToPolars(self, index):
        try:
            self.Convert(index)
            row = pl.DataFrame([self.info])
            self.df = pl.concat([self.df, row])
        except Exception as e:
            print(self.root + '--'+ str(index) + ' failed')
            print(traceback.print_exc())

    def SaveInfoToPolars(self):
        self.df = pl.DataFrame()
        for index in tqdm(self.data.index.tolist(), total=len(self.data.index.tolist()), desc="Saving to Polars DataFrame"):
            self.SaveRowToPolars(index)
        self.df.write_csv(os.path.join(self.params.data_path))
