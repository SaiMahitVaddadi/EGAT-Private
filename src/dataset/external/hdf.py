...
class HDFSaver:
    ...
    def SaveRowToHDF(self, hdf_store, index):
        try:
            self.Convert(index)
            row = pd.Series(self.info)
            hdf_store.append('data', row.to_frame().T, format='table', data_columns=True)
        except Exception as e:
            print(self.root + '--'+ str(index) + ' failed')
            print(traceback.print_exc())

    def SaveInfoToHDF(self):
        with pd.HDFStore(os.path.join(self.params.data_path), mode='w') as hdf_store:
            for index in tqdm(self.data.index.tolist(), total=len(self.data.index.tolist()), desc="Saving to HDF DataFrame"):
                self.SaveRowToHDF(hdf_store, index)
...
class HDFDataset(Dataset):
    ...
    def LoadHDFDataFrame(self):
        try:
            self.data = pd.read_hdf(os.path.join(self.params.data_path, 'data.h5'), 'data')
        except Exception as e:
            print(f"Failed to load HDF DataFrame from {self.params.data_path}")
            print(traceback.print_exc())
