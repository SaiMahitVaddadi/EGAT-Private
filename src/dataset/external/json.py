...
class JSONSaver:
    ...
    def SaveRowToJSON(self, index):
        try:
            ...
            json_file_path = os.path.join(rxntype_folder, f"{index}.json")
    ...
    def SaveInfoToJSON(self):
        for index in tqdm(self.data.index.tolist(), total=len(self.data.index.tolist()), desc="Saving to JSON files"):
