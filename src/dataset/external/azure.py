...
class PolarsSaver:
    ...
    def SaveRowToAzure(self, index):
        try:
            self.Convert(index)

            client = CosmosClient(self.params.azure_endpoint, self.params.azure_key)
            database_name = self.params.azure_database
            container_name = f"info_{index}"
            ...
            item = {k: str(v) for k, v in self.info.items()}
            item['id'] = str(index)
    ...
    def SaveInfoToAzure(self):
        for index in tqdm(self.data.index.tolist(), total=len(self.data.index.tolist()), desc="Saving to Azure"):
            self.SaveRowToAzure(index)
...
class PolarsDataset(Dataset):
    ...
    def LoadPolarsDataFrame(self):
        try:
            self.data = pd.read_csv(self.params.data_path, 'data')
        except Exception as e:
            print(f"Failed to load Polars DataFrame from {self.params.data_path}")
            print(traceback.print_exc())
