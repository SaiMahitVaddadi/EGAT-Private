...
class DaskSaver:
    ...
    def SaveRowToDask(self, ddf, index):
        try:
            self.Convert(index)
            row = pd.Series(self.info)
            if ddf is None:
                ddf = pd.DataFrame([row])
            else:
                ddf = ddf.append(row, ignore_index=True)
            return ddf
    ...
    def SaveInfoToDask(self):
        ...
        if self.params.size is None: 
            for index in tqdm(self.data.index.tolist(),total=len(self.data.index.tolist()), desc="Saving to Dask DataFrame"):
                ddf = self.SaveInfoToDask(ddf, index)
            ddf.to_parquet(os.path.join(self.params.data_path, 'dask_dataframe.parquet'))
...
class DaskDataset(Dataset):
    ...
    def LoadDaskDataFrame(self):
        try:
            self.data = dd.read_parquet(os.path.join(self.params.data_path, 'dask_dataframe.parquet'))
        except Exception as e:
            print(f"Failed to load Dask DataFrame from {self.params.data_path}")
            print(traceback.print_exc())
